...
This creates a gap in the literature: can we achieve a provable query reduction for knapsack that surpasses classical meet-in-the-middle, without relying on unproven heuristics or infeasible hardware?

We answer this question affirmatively. Our key contributions are:

\begin{enumerate}     \item A hybrid framework that couples LP-guided reduced-cost variable fixing with Montanaro's quantum branch-and-bound, analyzed specifically for knapsack. As Table~\ref{tab:related} shows, this is---to our knowledge---the only approach that is simultaneously provable, QRAQM-free, knapsack-specific, instance-adaptive, and tested by simulation.     \item Instance-dependent complexity bounds: $(\sqrt{T_{\max}} \, m \log V_{\max})$ where $T_{\text{LP}}$ is the LP-pruned B\&B tree size and $m \leq n$ is the core size after variable fixing.          \item The case $j \in F_0$ cannot occur: by the definition of reduced-cost fixing (Algorithm~\ref{alg:filter}), if $j \in F_0$ then forcing $j$ into the solution causes the LP upper bound to drop below the greedy lower bound $Z_{\text{LB}} \leq Z^*$. Since $Z^*$ is the optimal value and the LP bound is an upper bound, this implies no optimal solution includes $j$. \end{enumerate}
...
\multicolumn{5}{l}{\footnotesize $^\dagger$Provably never slower than classical B\&B, requiring fewer queries on LP-hard cores.} \\
...
The core contribution of this paper. We construct a classical B\&B tree over the $m$ core items using LP relaxation bounds at each node, then apply Montanaro's quantum walk~\cite{ref24} to search this tree in $$O(\sqrt{T_{\max}} \, m \log V_{\max})$$ time.
...
\textbf{Theorem 2 (Asymptotic Core Size Bounds).} \textit{Let $I_n$ be a knapsack instance of $n$ items, with core size $m = |C|$ remaining after LP reduced-cost fixing.} \begin{enumerate}     \item \textit{\textbf{Uncorrelated Random Model:} If weights $w_i$ and profits $v_i$ are drawn independently and uniformly from $(0, 1]$, and $W = c \sum w_i$ for some constant $c \in (0,1)$, then the expected core size satisfies $\mathbb{E}[m] \le O(\log^2 n)$. Consequently, the expected core ratio asymptotically vanishes: $\lim_{n \to \infty} \mathbb{E}[m/n] = 0$.}     \item \textit{\textbf{Subset-Sum Model:} If $v_i = w_i$ for all $i$ (perfect correlation), then deterministically $m = n$ and $\alpha = 1$.} \end{enumerate}

Observation 1 summarizes the empirical behavior of Phase 1 across standard distributions. Uncorrelated instances leave small cores, whereas strongly correlated and subset-sum instances resist variable fixing.
...
\textbf{Theorem 3 (Complexity).} \textit{Algorithm~\ref{alg:hybrid} solves the 0/1 knapsack problem in} \begin{equation}     T(n) = O(n \log n + \sqrt{T_{\text{LP}}\, m} \cdot \text{poly}(m, \log W'))     \label{eq:totalcomplexity} \end{equation} \textit{where $T_{\text{LP}}$ is the number of nodes visited by classical B\&B with LP bounding on the $m$ core items.}

\textit{Proof.} Phase~1 sorts the $n$ items by efficiency in $O(n \log n)$ time. Computing the greedy lower bound and the initial LP upper bound takes $O(n)$. Computing the $n$ ``leave-one-out'' or ``force-in'' LP bounds can be done dynamically by tracking the fractional split item in $O(1)$ amortized time per item, yielding a total Phase~1 cost of $O(n \log n)$. Phase~2 applies Montanaro's quantum walk to the B\&B tree of size $T_{\text{LP}}$ and depth at most $m$, requiring $\tilde{O}(\sqrt{T_{\text{LP}}\, m} \cdot \text{poly}(m, \log W'))$ oracle calls by Eq.~\ref{eq:montanaro}. Phase~3 is $O(n)$. The quantum phase dominates the worst-case scaling. \hfill$\blacksquare$

\textbf{Theorem 4 (Distributional Interpolation and Separation).} \textit{The hybrid algorithm guarantees asymptotic separation against both classical branch-and-bound and generic quantum search, adapting optimally across instance distributions:} \begin{enumerate}     \item \textit{\textbf{Easy Distributions (Uncorrelated):} The expected LP-pruned tree size is $\mathbb{E}[T_{\text{LP}}] = \text{poly}(n)$~\cite{beier2004random}. The hybrid algorithm terminates in expected $\text{poly}(n)$ queries, matching classical typical-case efficiency and achieving an expected super-polynomial speedup over generic Grover search ($\Omega(2^{n/2})$).}     \item \textit{\textbf{Hard Distributions (Subset-Sum):} Classical B\&B explores $T_{\text{LP}} = \Omega(2^{n/2})$ nodes for hard capacities. The hybrid algorithm searches this exact tree in $(\sqrt{T_{\max}} \, m \log V_{\max})$ queries, achieving a quadratic query reduction over classical B\&B.} \end{enumerate}

\textit{Proof.} (1) For uncorrelated uniform instances, the B\&B search space is heavily restricted by the tight $O(1/n)$ integrality gap. Beier and V\"ocking establish that the number of states evaluated by such LP-bounded search is polynomially bounded in expectation, yielding $\mathbb{E}[T_{\text{LP}}] = \text{poly}(n)$~\cite{beier2004random}. By Jensen's inequality, the expected quantum query complexity is $\mathbb{E}[\sqrt{T_{\text{LP}}\, m}] \le \sqrt{\mathbb{E}[T_{\text{LP}}]} = \text{poly}(n)$. Generic quantum search ignores the LP structure, querying the full $\Omega(2^n)$ space in $\Omega(2^{n/2})$ time. Thus, the hybrid algorithm is requires exponentially fewer queries than structure-ignorant quantum search. (2) For worst-case subset-sum, efficiencies are identical ($v_i/w_i = 1$). The LP upper bound equals the capacity $W$, providing no early pruning before reaching leaves. Classical B\&B explores an exponentially large tree $T_{\text{LP}} = \Omega(2^{n/2})$. Montanaro's quantum walk traverses this exact tree in $(\sqrt{T_{\max}} \, m \log V_{\max})$ queries. Consequently, our framework provides a provable instance-wise interpolation: avoiding the exponential penalty of generic quantum search on easy instances, and the exponential penalty of classical search on hard instances. \hfill$\blacksquare$

\subsection{Comparison with Prior Work}

Table~\ref{tab:comparison} summarizes the key differences.

\begin{table}[htbp] \caption{Comparison of Knapsack/Subset-Sum Algorithms} \label{tab:comparison} \begin{center} \begin{tabular}{lcccc} \toprule \textbf{Algorithm} & \textbf{Query/Time Bound} & \textbf{Space} & \textbf{Prov.} & \textbf{QRAQM} \\ \midrule Horowitz--Sahni & $O(2^{n/2})$ & $O(2^{n/2})$ & Yes & N/A \\ Grover & $O(2^{n/2})$ & $O(n)$ & Yes & No \\ Bernstein et al. & $O(2^{0.241n})$ & $O(2^{0.241n})$ & No$^\dagger$ & Yes \\ Helm \& May & $O(2^{0.226n})$ & $O(2^{0.226n})$ & No$^\dagger$ & Yes \\ Montanaro B\&B & $O(\sqrt{T}\,d)$ & $O(\text{poly})$ & Yes & No \\ QTG+AA & $O(\sqrt{N_f})$ & $O(\text{poly})$ & Yes & No \\ \textbf{Ours} & $O(\sqrt{T_{\max}}\,m\log V_{\max})$ & $O(\text{poly})$ & \textbf{Yes} & \textbf{No} \\ \bottomrule \multicolumn{5}{l}{\footnotesize $^\dagger$Relies on Heuristic 2, shown unsound by \cite{bonnetain2020improved}.} \\ \multicolumn{5}{l}{\footnotesize $T$: full B\&B tree. $T_{LP}$: LP-pruned tree ($T_{LP} \ll T$). $N_f$: feasible states.} \end{tabular} \end{center} \end{table}

Our algorithm achieves $\sqrt{T_{LP}}$ where $T_{LP} \ll T$ due to LP pruning, combining the structural advantage of LP bounds with the query reduction of Montanaro's framework.
...
For the completed instances, we provide computational evidence for three primary spectral endpoints:
\begin{enumerate}
    \item \textbf{Unmarked Effective Spectral-Gap Inequality:} satisfied for 400/400 completed unmarked trees.
    \item \textbf{Marked Phase-Zero Weight:} satisfied for 489/489 completed marked trees.
    \item \textbf{Phase-Estimation Separation:} 889/889 analytical detection criterion agreement with known marked/unmarked status.
\end{enumerate}
...
At the small values of $n$ tested ($n \leq 24$), Montanaro's polynomial overhead in depth ($\text{poly}(d)$) can dominate the $\sqrt{T}$ query reduction, particularly for subset-sum where $T_{\text{LP}}$ is still moderate. However, for large-scale instances ($n \geq 100$), strongly correlated instances will have $T_{\text{LP}} = rapid growth$ for some $c > 0$, making the quantum query reduction $\sqrt{T_{\text{LP}}\, m} = scaling$ significant compared to the $\text{poly}(n)$ overhead. The scaling intersection point---where quantum operations become fewer than classical---occurs at approximately $T_{\text{LP}} \geq \text{poly}(m)^2$, which our strongly correlated instances approach at $n = 24$.

\subsection{Limitations}

\textbf{Hardware requirements.} The quantum phase requires coherent execution of $O(\sqrt{T_{\text{LP}}\, m})$ quantum walk steps, each involving an LP bounding oracle with reversible marked and threshold predicates. For large core sizes ($m \geq 5$), this demands fault-tolerant quantum hardware, as our IBM experiments demonstrate.
...
\textbf{Small-$n$ overhead.} At $n \leq 24$, the Montanaro $\text{poly}(d)$ overhead can make the quantum algorithm slower than classical B\&B for easy instances. The quantum query reduction is asymptotic and becomes pronounced as $n$ grows and $T_{\text{LP}}$ increases exponentially.

\textbf{Pseudo-polynomial regime.} For instances with small integer coefficients, dynamic programming runs in $O(nW)$ and dominates: our theoretical comparison to branch-and-bound is meaningful only in the large-coefficient regime where DP is infeasible. We make this scope explicit rather than claiming universal speedup over all classical methods.

\textbf{Detection reliability and simulability.} Our phase-estimation precision constant is calibrated empirically. Walk simulation restricts the spectral audit to trees with $T_{\text{LP}} \le 2000$ nodes. \textbf{Oracle Completeness.} Proposition~1 implements the reversible evaluation/threshold predicate $V$, not the LP heuristic $h$. We explicitly scope our resource estimation to the reversible evaluation/threshold predicate $V$ only. The reversible LP heuristic $h$ is not synthesized, so no full-algorithm gate-complexity bound is claimed.
...
We presented a hybrid algorithm that couples LP-guided reduced-cost variable fixing with Montanaro's quantum branch-and-bound for the 0/1 knapsack problem---to our knowledge the only approach combining provability, gate-model feasibility, knapsack-specific LP fixing, instance adaptivity, and tested query complexity (Table~\ref{tab:related}). Our hybrid algorithm achieves complexity $(\sqrt{T_{\max}} \, m \log V_{\max})$, where $T_{\text{LP}}$ is the LP-pruned branch-and-bound tree size and $m = \alpha n$ is the core size after variable fixing. We proved three key results:

\begin{enumerate}     \item \textit{Correctness}: reduced-cost fixing is exact---no heuristic assumptions are involved---and Montanaro's quantum walk finds the optimal B\&B solution with high probability.     \item \textit{Instance-dependent synergy}: for uncorrelated instances, LP fixing achieves $\alpha \approx 0.02$ at scale ($n = 10{,}000$), reducing the effective search to a trivial core; for strongly constrained instances where classical B\&B explores exponentially many nodes, the quantum phase provides a quadratic query reduction relative to classical traversal of the same backtracking tree under the Montanaro oracle-model assumptions.     \end{enumerate}
