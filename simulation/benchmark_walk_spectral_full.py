...
def run_spectral_benchmark():
    ...
    out_filename = "quantum_walk_spectral_full.json"
    
    # Pre-declared endpoints and thresholds
    manifest = {
        "git_commit": git_hash,
        ...
        "seeds": [f"1000 * n + t (t=0..{TRIALS_FULL-1})"],
        "pe_constant": PE_CONSTANT,
        "k_repeats": K_REPEATS,
        "max_nodes": MAX_NODES,
        "output_filename": out_filename,
        "endpoints": {
            "unmarked_effective_spectral_gap": "mu(Phi_c) <= Phi_c^2 * T_LP * m / 4",
            "marked_phase_zero_weight": "w_0 >= 1/2",
            "analytical_detection_agreement": "decided_marked == actual_marked"
        },
        "test_parameters": {
            "c_values": [0.125, 0.25, 0.5, 1.0, 2.0],
            "epsilon_values": [1e-8, 1e-10, 1e-12],
            "phase_precisions": ["s-2", "s-1", "s", "s+1"],
            "majority_threshold": "3/8 single-shot acceptances"
        }
    }

    print("=== FULL SPECTRAL MANIFEST ===")
                ...
                for label, tau in [("marked", tau_marked), ("unmarked", tau_unmarked)]:
                    ...
                    if is_marked:
                        ...
                        for eps in [1e-8, 1e-10, 1e-12]:
                            w_0 = float(sum(overlaps[i] for i in range(T_LP) if abs(phases[i]) <= eps))
                            record_copy["w_0"][f"eps_{eps}"] = w_0
